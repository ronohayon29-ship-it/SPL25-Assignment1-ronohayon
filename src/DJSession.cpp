...
bool DJSession::load_track_to_mixer_deck(const std::string& track_title) {
    ...
    int val = mixing_service.loadTrackToDeck(*track);
    if(val==0){
        stats.deck_loads_a++;
        stats.transitions++;
    }
        ...
        std::cerr << "[ERROR] Failed to load track to deck\n";
        stats.errors++;
        return false;
    }
    return true;
    ...
    if(!play_all){
        while(true){
            ...
            if(load_playlist(playlist_name)){
                for(int i = (int)track_titles.size() - 1; i>=0; i--){
        ...
        for(std::string title: playlist_names){
            if(load_playlist(title)){
                for(int i = (int)track_titles.size() - 1; i>=0; i--){
                    std::cout << "\n--- Processing: " << track_titles[i] << " ---\n";
                    stats.tracks_processed++;
                    load_track_to_controller(track_titles[i]);
